...
`protocols/exports/` 预留给协议导出产物。当前代码已完成 Stage 1–7，包括 domain contracts、Gate Arbiter、首个协议场景、单一 Qwen adapter、服务端内存 Hidden Session、text-first interview runtime，以及 Chrome 优先的沉浸式语音回答 shell；尚未实现 Semantic Evaluator 编排、Hard Gate 或 Repair runtime。

浏览器现可完成 Session 创建、开始回答、麦克风录入、实时语音转写、真实音量反馈、transcript 自动同步、版本化 checkpoint 和主动完成。语音不可用时可切换到文本备用模式。下一阶段在同一个冻结 QuestionPlan 上进行 Semantic Evaluator/Gate Arbiter 编排；持久化和双模型优化仍延后。
