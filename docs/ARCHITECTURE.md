...
The repository keeps portable interview assets separate from product execution. The current code contains the scaffold and Stage 1 domain contracts; this document freezes the remaining text-first MVP boundaries rather than claiming those runtime features are implemented.
